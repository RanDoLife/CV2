...
def image_callback(msg: Image, cv_bridge: CvBridge) -> None:
    ...
    # A: Формат изображения определяется полем msg.encoding. В данном случае, оно может быть 'mono8' (монохромное, 8-битное),
    # 'bgr8' (трехканальное изображение в формате BGR) или другим поддерживаемым форматом.
    ...
    cv2.waitKey(1)  # Обновление окна без задержки
...
def main() -> None:
    ...
    # A: rospy.spin() запускает бесконечный цикл, который позволяет обработчику подписки
    # продолжать получать и обрабатывать сообщения из указанного топика ROS.
    rospy.spin()
